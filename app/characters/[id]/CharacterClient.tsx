...
import { updateCharacterHpAction, updateWeaponsAction } from "@/app/actions/character";
...
export default function CharacterClient({ character }: { character: any }) {
  ...
  const [inspectedItem, setInspectedItem] = useState<{ title: string, desc: string, type: string } | null>(null);
  
  const [isRolling, setIsRolling] = useState(false);
  const [rollResult, setRollResult] = useState<{ dice: number, result: number } | null>(null);
  const [equippedWeapons, setEquippedWeapons] = useState<any[]>(character.weapons || []);

  const profBonus = Math.ceil(1 + character.level / 4);
  const mods = { STR: getMod(character.stats.STR), DEX: getMod(character.stats.DEX), CON: getMod(character.stats.CON), INT: getMod(character.stats.INT), WIS: getMod(character.stats.WIS), CHA: getMod(character.stats.CHA) };
  ...
  const updateHpInDb = async (newHp: number) => { setIsUpdatingHp(true); setCurrentHp(newHp); await updateCharacterHpAction(character._id.toString(), newHp); setIsUpdatingHp(false); };
  const handleHeal = () => { if (!hpInput) return; updateHpInDb(Math.min(character.hpMax, currentHp + Number(hpInput))); setHpInput(""); };
  const handleDamage = () => { if (!hpInput) return; updateHpInDb(Math.max(0, currentHp - Number(hpInput))); setHpInput(""); };
  const handleLongRest = () => { if (confirm("Lakukan Long Rest? HP akan pulih penuh.")) updateHpInDb(character.hpMax); };

  const rollDice = (sides: number) => { setIsRolling(true); setRollResult(null); setTimeout(() => { setRollResult({ dice: sides, result: Math.floor(Math.random() * sides) + 1 }); setIsRolling(false); }, 600); };
  
  const toggleEquip = async (itemName: string) => {
    const weaponData = getWeaponData(itemName); if (!weaponData) return;
    const newWeapons = equippedWeapons.some(w => w.name === itemName) ? equippedWeapons.filter(w => w.name !== itemName) : [...equippedWeapons, { ...weaponData, name: itemName }];
    setEquippedWeapons(newWeapons); await updateWeaponsAction(character._id.toString(), newWeapons);
  };

  return (
    <main className="min-h-screen bg-[#0f111a] text-slate-200 font-sans pb-32 selection:bg-red-500/30 relative printable-area">
      
      {/* HEADER: Navigasi dan PDF Download */}
      <header className="bg-[#181b26] border-b border-[#2d3245] sticky top-0 z-30 shadow-md no-print">
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* TOMBOL PDF MUNCUL DI SINI */}
            <button onClick={() => window.print()} className="bg-purple-900/30 hover:bg-purple-900/60 text-purple-400 text-xs font-bold px-4 py-1.5 rounded-md border border-purple-900/50 transition-colors active:scale-95 ml-2 flex items-center gap-2">📄 DOWNLOAD PDF</button>
          </div>
        </div>
      </header>
        ...
        <div className="bg-[#181b26] rounded-t-xl border border-[#2d3245] border-b-0 p-4 flex flex-col xl:flex-row items-center xl:items-end justify-between gap-6 relative overflow-hidden print-border">
          <div className="flex flex-col md:flex-row items-center gap-6 z-10 w-full xl:w-auto">
            ...
            <div className="text-center md:text-left w-full">
              ...
              <p className="text-sm font-bold text-slate-400 mt-1">{character.race} • {character.class} {character.level}</p>
            </div>
          </div>
          {/* KOTAK HP (Tidak dicetak ke PDF agar bisa ditulis tangan) */}
          <div className="bg-[#0f111a] border-2 border-red-900/50 rounded-xl p-3 flex gap-4 items-center min-w-[300px] z-10 shadow-lg no-print">
            <div className="flex flex-col gap-1.5 w-20">
              ...
              <div className="flex gap-1 h-6"><button onClick={handleHeal} className="flex-1 text-[9px] font-black bg-green-900/40 text-green-400 rounded border border-green-800/50">Heal</button><button onClick={handleDamage} className="flex-1 text-[9px] font-black bg-red-900/40 text-red-400 rounded border border-red-800/50">Dmg</button></div>
            </div>
            <div className="flex-1 text-center"><span className="block text-[10px] font-black text-red-500 uppercase tracking-widest mb-1">Hit Points</span><div className="flex justify-center items-baseline gap-2"><span className="text-4xl font-black text-white">{currentHp}</span><span className="text-xl font-bold text-slate-500">/ {character.hpMax}</span></div></div>
          </div>
        </div>
            ...
            <div className="bg-[#181b26] rounded-xl border border-[#2d3245] shadow-sm flex flex-col print-border">
              ...
              <div className="flex flex-col py-2">
                {Object.entries(SKILL_MAP).map(([skill, stat]) => {
                  ...
                  return (
                    <div key={skill} className={`flex items-center justify-between px-4 py-2 ${isProf ? 'bg-[#2d3245]/10' : ''}`}>
            ...
            <div className="bg-[#181b26] rounded-xl border border-[#2d3245] shadow-sm overflow-hidden flex flex-col print-border print-hide-tabs">
              <div className="flex overflow-x-auto border-b border-[#2d3245] bg-[#0f111a] no-print">
                {["ACTIONS", "SPELLS", "INVENTORY", "FEATURES"].map((tab) => (
                  <button key={tab} onClick={() => { setActiveTab(tab); setInspectedItem(null); }} className={`px-4 py-3 text-xs font-black uppercase tracking-widest whitespace-nowrap border-b-2 ${activeTab === tab ? 'text-red-500 border-red-500 bg-[#181b26]' : 'text-slate-500 border-transparent hover:text-slate-300'}`}>{tab}</button>
                ))}
              </div>
              <div className="flex flex-col xl:flex-row h-full">
                      ...
                      <div className="space-y-2">
                        {equippedWeapons && equippedWeapons.length > 0 ? (
                          equippedWeapons.map((w: any, idx: number) => {
                            ...
                            return (
                              <div key={idx} onClick={() => setInspectedItem({ title: w.name, desc: `Weapon. Menggunakan dadu ${w.damage} untuk kerusakan.`, type: 'Weapon' })} className="grid grid-cols-12 gap-2 bg-[#0f111a] p-3 rounded-lg border border-[#2d3245] items-center cursor-pointer print-border">
                                <div className="col-span-5"><span className="block text-sm font-bold text-white mb-1 print-text-black">{w.name}</span><span className="text-[10px] text-slate-500 italic">{w.isRanged ? 'Ranged' : 'Melee'}</span></div><div className="col-span-2 text-center text-xs text-slate-400">{w.isRanged ? '120 ft' : '5 ft'}</div><div className="col-span-2 text-center"><span className="bg-[#181b26] border border-[#3e455c] text-white text-xs px-2 py-1 rounded print-border print-text-black">{formatMod(profBonus + statMod)}</span></div><div className="col-span-3 text-center"><span className="text-xs font-bold text-slate-300 print-text-black">{w.damage}</span></div>
                              </div>
                            );
                          })
                        ) : (<div className="text-center py-4 text-xs font-bold text-slate-500">Belum ada senjata Equip.</div>)}
                      </div>
                    </div>
                  )}
                      ...
                      <div className="space-y-2">
                        {character.equipment && character.equipment.length > 0 ? (
                          character.equipment.map((item: string, idx: number) => {
                            ...
                            return (
                              <div key={idx} className="bg-[#0f111a] p-3 rounded-lg border border-[#2d3245] flex items-center justify-between group hover:border-[#3e455c] print-border">
                                <div className="flex items-center gap-3 flex-1" onClick={() => setInspectedItem({ title: item, desc: "Item.", type: weaponData ? "Weapon" : "Item" })}>
                                ...
                                {weaponData && (
                                  <button onClick={(e) => { e.stopPropagation(); toggleEquip(item); }} className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded transition-all active:scale-95 no-print ${isEquipped ? 'bg-red-900/40 text-red-400 border border-red-900/50 hover:bg-red-900/60' : 'bg-[#181b26] text-slate-400 border border-[#3e455c]'}`}>
                ...
                {/* INSPECTOR (Sembunyikan saat di Print) */}
                {inspectedItem && (
                  <div className="p-6 bg-[#0a0c13] xl:w-1/2 relative animate-fadeIn border-t xl:border-t-0 border-[#2d3245] no-print">
                    <button onClick={() => setInspectedItem(null)} className="absolute top-4 right-4 w-6 h-6 rounded-full bg-[#181b26] border border-[#2d3245] text-slate-500 flex items-center justify-center text-xs font-bold hover:text-white">✕</button>
                    <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${inspectedItem.type === 'Weapon' ? 'text-red-400 border-red-900/50 bg-red-900/20' : 'text-slate-400 border-slate-700 bg-slate-800'}`}>{inspectedItem.type}</span>
                    <h4 className="text-xl font-black text-white mt-3 mb-4 leading-tight">{inspectedItem.title}</h4>
                    <p className="text-sm text-slate-300 leading-relaxed font-medium">{inspectedItem.desc}</p>
                    
                    {inspectedItem.type === 'Weapon' && (
                       <button onClick={() => toggleEquip(inspectedItem.title)} className={`mt-6 w-full py-3 rounded-xl font-black uppercase tracking-widest text-xs transition-colors border ${equippedWeapons.some(w => w.name === inspectedItem.title) ? 'bg-red-900/20 border-red-900/50 text-red-400 hover:bg-red-900/40' : 'bg-red-600 border-red-500 text-white hover:bg-red-500'}`}>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 no-print">
        <div className="bg-[#181b26]/90 backdrop-blur-md border border-[#2d3245] shadow-[0_10px_40px_rgba(0,0,0,0.8)] p-2 rounded-2xl flex items-center gap-2 animate-fadeIn hover:border-red-900/50 transition-colors">
           ...
           {[4, 6, 8, 10, 12, 20].map((dice) => (
              <button key={dice} onClick={() => rollDice(dice)} className={`w-12 h-12 flex items-center justify-center rounded-xl font-black text-lg transition-all active:scale-90 shadow-inner ${dice === 20 ? 'bg-red-600 hover:bg-red-500 text-white shadow-red-900/50' : 'bg-[#0f111a] border border-[#3e455c] text-slate-300 hover:text-white hover:border-slate-400'}`}>d{dice}</button>
           ))}
        </div>
      </div>
      ...
      {/* --- CSS PRINT STYLE (D&D BEYOND INK-FRIENDLY FORMAT) --- */}
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* THE MAGIC: CSS UNTUK MENCETAK KE KERTAS ATAU PDF */
        @media print {
          @page { margin: 1.5cm; size: A4 portrait; }
          body, .printable-area { background: #ffffff !important; color: #000000 !important; }
          .no-print { display: none !important; }
          
          /* Memaksa browser mencetak garis pinggir dan menghitamkan teks */
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; box-shadow: none !important; }
          ...
          /* Tampilkan semua tab list secara berurutan di kertas, sembunyikan navigasi tab */
